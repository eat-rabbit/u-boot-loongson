 ...
 * 例子
 * 以ls2k1000为例
 * 金龙板 可以 切换 dvo0显示 和 多串口
 * ls2k1000_jl_vaild_product_set ls2k1000_jl_vaild_product_option_set
 * ls2k1000_jl_vaild_product_set 指的是 smbios 的 baseboard 的 product 属性值 最终目的是 kernel 选择 dts 的 属性值
 * ls2k1000_jl_vaild_product_option_set 指的是 菜单项 的展示
 * 上面的 两个 数组 都需要 "" 作为 第一个 元素 这是因为 0 这个值代表默认(也就是到时候不会重新设置 product 的 值)
 * 同时最后一个元素应为 NULL
 * ls2k1000_jl_vaild_product_option_set 菜单 选项 中 =1 =2 这是固定的。= 前面的是 展示文本 后面的是执行函数的值 set_board_kernel_product_id
 *
 * product_set_ptr_set
 * product_option_set_ptr_set
 * 上面两个 数组指针 就是容纳 ls2k1000_jl_vaild_product_set 或者 ls2k1000_jl_vaild_product_option_set 的数组
 * 注意 顺序 应该是和 board_name_set 的 一致。
 */
#if defined(CONFIG_SOC_LS2K500)
static char* generic_null_option_set[] = {"no option=""\0", NULL};
static char* board_name_set[] = {LS2K500_EVB_BOARD_NAME,
									LS2K500_HL_BOARD_NAME,
									LS2K500_MINI_DP_BOARD_NAME,
									LS2K500_MODI_HCT_BOARD_NAME,
									NULL};
...
static char* ls2k1000_jl_vaild_product_set[] = {"", LS2K1000_JL_DESC, "LS2K1000-JL-MB-MU", NULL};
static char* ls2k1000_jl_vaild_product_option_set[] = {"", "jinlong board default(use dvo0)=1", "jinlong board multi uart(not use dvo0)=2", NULL};

static char* ls2k1000_dp_vaild_product_set[] = {"", LS2K1000_DP_DESC, "LS2K1000-DP-TEST", NULL};
static char* ls2k1000_dp_vaild_product_option_set[] = {"", "ls2k1000 DP board default=1", "ls2k1000 DP board test mode=2", NULL};

static char* board_name_set[] = {LS2K1000_JL_BOARD_NAME, LS2K1000_DP_BOARD_NAME, NULL};
static char** product_set_ptr_set[] = {ls2k1000_jl_vaild_product_set, ls2k1000_dp_vaild_product_set};
static char** product_option_set_ptr_set[] = {ls2k1000_jl_vaild_product_option_set,ls2k1000_dp_vaild_product_option_set};
#elif defined(CONFIG_SOC_LS2P500)
static char* generic_null_option_set[] = {"no option=""\0", NULL};
static char* board_name_set[] = {LS2P500_EVB_BOARD_NAME,
									NULL};
static char** product_set_ptr_set[] = {NULL, NULL, NULL};
static char** product_option_set_ptr_set[] = {generic_null_option_set, generic_null_option_set, generic_null_option_set};
#elif defined(CONFIG_SOC_LS2K300)
static char* generic_null_option_set[] = {"no option=""\0", NULL};
static char* board_name_set[] = {LS2K300_MINI_DP_BOARD_NAME,
									NULL};
static char** product_set_ptr_set[] = {NULL, NULL, NULL};
static char** product_option_set_ptr_set[] = {generic_null_option_set, generic_null_option_set, generic_null_option_set};
#endif
